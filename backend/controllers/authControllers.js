...
/* ======================================================================
        Register Sub Admin (/api/v1/register/sub/admin) (req : POST)
   ======================================================================= */
exports.registerSubAdmin = catchAsyncError(async (req, res, next) => {
  const { userName, password } = req.body;
  const nameUser = await SubAdmin.findOne({ userName });
  if (nameUser) {
    return next(new ErrorHandler("This Admin already exist.", 400));
  }
  await SubAdmin.create({ userName, password });
  res.status(200).json({
    success: true,
    message: "HR Register Successfully",
  });
});
